...
module.exports = function sync(options, instructions) {
  ...
  instructions.forEach(function(instruction) {
    ...
    instruction.repositories.forEach(function(repository) {
      ...
      var options = instruction.options || {};
      var ignoreLabels = options.ignoreLabels || [];
      var ignoreMilestones = options.ignoreMilestones || [];

      if (instruction.milestones.length && ignoreMilestones.indexOf(repository) === -1) {
      ...
      if (instruction.labels.length && ignoreLabels.indexOf(repository) === -1) {
